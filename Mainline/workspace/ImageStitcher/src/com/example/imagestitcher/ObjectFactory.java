	...
	public Object loadObject(String fileName, Context context) throws FileNotFoundException{
		...
		if(Environment.MEDIA_MOUNTED.equals(state) || Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)){
			...
			if(fileName.contains(".obj")){
				...
				while(input.hasNextLine()){
					if(input.hasNext(Pattern.compile("v"))){
						if(!readingVertices){
							...
							Toast.makeText(context, "VERTICES STARTED", Toast.LENGTH_LONG).show();
						...
						if(!readingTextures && readingVertices){
							Toast.makeText(context, "VERTICES DONE", Toast.LENGTH_LONG).show();
						...
						if(readingTextures && readingVertices)
							readingFaces = true;
						if(readingFaces){
							Toast.makeText(context, "FINISHED LOADING MODELS", Toast.LENGTH_LONG).show();
							Log.w("MESSAGE", "DONE!");
							break;
						}
			    ...
			    myObj.vertices = FloatBuffer.wrap(floatArray);
			    Log.w("MESSAGE", "DONE!!!");
			    return myObj;
			}
		}
